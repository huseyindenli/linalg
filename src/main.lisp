...
(defun detect-keyboard-pressed ()
  (al:with-current-keyboard-state state
    (when (al:key-down state :escape) ;; ESC terminates the loop
      (print "escape"))
    (when (al:key-down state :space) ;; ESC terminates the loop
      (print "space"))))
...
(cffi:defcallback %main :int ((argc :int) (argv :pointer))
  ...
  (handler-bind
    ...
    (let ((display (al:create-display +window-width+ +window-height+))
      ...
      (setf cl-opengl-bindings:*gl-get-proc-address* #'al:get-opengl-proc-address)
      (when (cffi:null-pointer-p display)
        (error "Initializing display failed"))
      (al:inhibit-screensaver t)
      (al:set-window-title display "ut3")
