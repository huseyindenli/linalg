...
case $1 in
    linux)
        do_build
        linuxdeploy --appimage-extract-and-run --executable=bin/linalg \
                    --custom-apprun=package/AppRun \
                    --icon-file=package/icon.png \
                    --desktop-file=package/linalg.desktop \
                    --appdir=appimage $(find bin -name "lib*" -printf "-l%p ")
        cp bin/linalg appimage/usr/bin
        cp -R Resources appimage/usr
        appimagetool --appimage-extract-and-run --comp xz -g appimage "linalg-${VERSION}.AppImage"
        ;;
    ...
    macos)
        ...
        xxd -ps -c1 bin/linalg | tr -d '\n' | sed "s/$replace_fr/$replace_to/" | fold -w 2 | xxd -r -p > "$contents/MacOS/linalg"
        chmod +x "$contents/MacOS/linalg"

        hdiutil create -quiet -srcfolder "$bundle" out.dmg
        # NOTE: ULMO = lzma compression = Catalina+ only
        hdiutil convert -quiet out.dmg -format ULMO -o "linalg-${VERSION}.dmg"
        rm out.dmg
        ;;
